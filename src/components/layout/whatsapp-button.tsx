...
export function WhatsAppButton() {
  ...
  useEffect(() => {
    // Mostrar boton despues de 2 segundos
  ...
  // Detectar cuando la seccion Contact esta visible
  useEffect(() => {
  ...
  // Mostrar tooltip despues de 5 segundos, solo una vez
  useEffect(() => {
    if (isVisible && !isContactVisible) {
      const tooltipTimer = setTimeout(() => {
        setShowTooltip(true);
        // Ocultar tooltip despues de 4 segundos
  ...
  return (
    ...
    >
      ...
      {/* Boton - FAB with consistent design system transitions */}
      <a
